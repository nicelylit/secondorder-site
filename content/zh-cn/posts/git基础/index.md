...
后面三句先不解释含义，后面会逐步说明。只说明第一句`git init`，表示在当前目录下，生成`.git`仓库来记录该目录下的项目。
...
**当执行`git status`时候，Git总把一些不相关的文件列在未记录的文件中，怎么样能够不看到这些文件，这些文件都不会被记录？**
...
实际上`git log`还有查找特定字段`--grep`，自定义输出格式`--pretty`，显示单行`--oneline`等等一些更加复杂的命令，具体情况可参考[Git的帮助手册](http://git-scm.com/docs)。
...
```bash
$ git push <remote_brach> <tag_version>
```
