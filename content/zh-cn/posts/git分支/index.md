...
? ? ?$ git checkout tracking\_branch

? ? ?$ git merge origin/tracking\_branch
...
? ? ?$ git checkout -b <branch\_name> <remote\_name>/<branch\_name>
...
? ? ?$ git checkout --track <remote\_name>/<branch\_name>

? ? ?如果要改变本地分支追踪的远程分支，可以执行

? ? ?$ git checkout -u <remote\_name>/<branch\_name>
